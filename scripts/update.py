...
def update_pb(firmware, version, log=None):
    res = subprocess.run(['dfu-util', '-E', '1', '-d', '1bda:0010', '-D', firmware])

    sleep(1)

    try:
        s = serial.serial_for_url('hwgrep://1bda:0010', timeout=3)
        s.write(b'*IDN?\n')
        identity_raw = s.readline()

        identity = identity_raw.decode('utf-8').strip().split(':')
        if identity[0] == 'Student Robotics' and identity[1] == 'PBv4B':
...
def main():
    ...
    try:
        while True:
            ...
            update_pb(args.file, args.version, args.serial_log)
